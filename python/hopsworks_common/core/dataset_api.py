...
class DatasetApi:
    ...
    def download(
        ...
        # Raises
            `hopsworks.client.exceptions.RestAPIError`: If unable to download the file
        """
    ...
    def upload(
        ...
        # Raises
            `hopsworks.client.exceptions.RestAPIError`: If unable to upload the file or directory
        """
    ...
    def exists(self, path: str):
        ...
        # Raises
            `hopsworks.client.exceptions.RestAPIError`: If unable to check existence for the path
        """
        try:
            self._get(path)
            return True
    ...
    def remove(self, path: str):
        ...
        # Arguments
            path: path to remove
        # Raises
            `hopsworks.client.exceptions.RestAPIError`: If unable to remove the path
        """
        _client = client.get_instance()
        path_params = ["project", _client._project_id, "dataset", path]
    ...
    def mkdir(self, path: str):
        ...
        # Raises
            `hopsworks.client.exceptions.RestAPIError`: If unable to create the directory
        """
        _client = client.get_instance()
        path_params = ["project", _client._project_id, "dataset", path]
        query_params = {
    ...
    def copy(self, source_path: str, destination_path: str, overwrite: bool = False):
        ...
        # Raises
            `hopsworks.client.exceptions.RestAPIError`: If unable to perform the copy
        """
        if self.exists(destination_path):
            if overwrite:
                if "datasetType" in self._get(destination_path):
    ...
    def move(self, source_path: str, destination_path: str, overwrite: bool = False):
        ...
        # Arguments
            ...
            overwrite: overwrite destination if exists
        # Raises
            `hopsworks.client.exceptions.RestAPIError`: If unable to perform the move
        """
        if self.exists(destination_path):
            if overwrite:
                if "datasetType" in self._get(destination_path):
