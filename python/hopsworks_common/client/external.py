...
class Client(base.Client):
    ...
    def _materialize_certs(self):
        # Create user-specific subdirectory: {cert_folder_base}/{host}/{project}/{username}
    ...
    def _makedirs_with_sticky_bit(self) -> None:
        """Create cert directories with sticky bit for multi-user support.

        Sets 1777 permissions on host and project directories so multiple
        system users can create files, but only owners can rename/delete their own.
        The user-specific directory is created with 0700 permissions (owner only).
        """
